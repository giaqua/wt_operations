...
frappe.query_reports["Daily Off-Spec Archive Report"] = {
    filters: [
        {
            fieldname: "project",
            label: __("Project"),
		...
		{
			"fieldname": "show_other_info",
			"label": __("Show Other Info"),
			"fieldtype": "Check",
			"default": 0
		}
    ],
    ...
    onload: function (report) {
        report.page.add_inner_button(
            __("Print Report"),
            function () {
                print_archive_offspec_report(report);
            },
            __("Actions")
        );
    },
...
// ---------------------------------------------------------------------------
// Branded print view - identical approach to Daily Off-Spec Report:
// columns/data/totals all come from the server (get_print_data), and the
// <thead> "display: table-header-group" trick repeats the header (with
// company logo) on every printed page.
// ---------------------------------------------------------------------------
...
// "your_app.your_module.report.daily_off_spec_archive_report.daily_off_spec_archive_report.get_print_data"
const ARCHIVE_PRINT_DATA_METHOD =
    "wt_operations.wt_operations.report.daily_off_spec_archive_report.daily_off_spec_archive_report.get_print_data";

function print_archive_offspec_report(report) {
    const filters = frappe.query_report.get_filter_values();

    frappe.dom.freeze(__("Preparing print view..."));

    frappe.call({
        method: ARCHIVE_PRINT_DATA_METHOD,
        args: { filters },
        callback: function (r) {
            frappe.dom.unfreeze();
            ...
            render_archive_print_window(columns, data, totals, server_filters, company || {}, is_arabic);
        },
        error: function () {
            frappe.dom.unfreeze();
            frappe.msgprint(
...
function render_archive_print_window(columns, data, totals, filters, company, is_arabic) {
    const logo_url = company.company_logo ? frappe.urllib.get_full_url(company.company_logo) : "";
    const company_name = company.company_name || "";

    const title = is_arabic
        ? "التقرير اليومي لمخالفة المواصفات (الأرشيف)"
        : "Daily Off-Spec Report (Archive)";
