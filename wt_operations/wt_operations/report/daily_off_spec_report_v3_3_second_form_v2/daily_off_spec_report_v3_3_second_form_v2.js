...
frappe.query_reports["Daily Off-Spec Report V3-3 Second Form v2"] = {
    filters: [
        {
            fieldname: "project",
            label: __("Project"),
    ...
    onload: function (report) {
        ...
        // ---- Excel export buttons (mirrors the print buttons above) ----
...
const PRINT_DATA_METHOD =
    "wt_operations.wt_operations.report.daily_off_spec_report_v3_3_second_form_v2.daily_off_spec_report_v3_3_second_form_v2.get_print_data";

// Excel export endpoint - same dotted-path convention as PRINT_DATA_METHOD above.
const EXCEL_EXPORT_METHOD =
    "wt_operations.wt_operations.report.daily_off_spec_report_v3_3_second_form_v2.daily_off_spec_report_v3_3_second_form_v2.export_excel";
...
// ---------------------------------------------------------------------------
// Excel export
// Triggers a GET against the whitelisted export_excel method so the browser
// handles the binary download directly (no CSRF concerns on GET). Filters
// are passed the same way as the print flow, plus add_monthly_subtotals.
// ---------------------------------------------------------------------------
...
function export_daily_offspec_excel(report, filter_overrides, options) {
    ...
    const query_string = Object.keys(args)
        .map((key) => `${encodeURIComponent(key)}=${encodeURIComponent(args[key])}`)
        .join("&");

    window.open(`/api/method/${EXCEL_EXPORT_METHOD}?${query_string}`);
}
