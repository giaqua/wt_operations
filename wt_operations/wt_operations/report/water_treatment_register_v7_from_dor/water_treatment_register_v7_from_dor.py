...
#   - Daily Operation Report -> Influent Parameters Table / Effluent
#     Parameters Table (child tables on the DOR itself; each row carries
#     `parameter` + `actual_value`). Inlet readings come from Influent
#     Parameters Table, Outlet readings come from Effluent Parameters
#     Table - both keyed directly off the DOR, no separate sample document
#     lookup/matching is needed anymore.
...
#     Daily Off-Spec calc - always keyed off the INLET (Influent) COD value,
...
#
# Inlet / Outlet parameter readings:
#   - Previously resolved via a separate "Project Operation Water Sample"
#     document (single-source per doc, tagged Inlet/Outlet, matched to a
#     DOR by site == project and sampling_date == date, with per-parameter
#     values on a "Sample Collection Details" child table).
#   - Now resolved directly from the DOR itself: Influent Parameters Table
#     (Inlet) and Effluent Parameters Table (Outlet) are child tables of
#     Daily Operation Report, each row carrying `parameter` +
#     `actual_value`. No matching/lookup against a separate document is
#     needed - the rows are already attached to the DOR being reported on.
#   - get_influent_parameter_map() / get_effluent_parameter_map() batch-
#     fetch every row across all DORs in range in a single query each (one
#     for Influent, one for Effluent), keyed by dor.name -> {parameter:
#     actual_value}, mirroring the batching pattern used for Fresh Water
#     Consumption below.
#   - As before, on a zero-flow day (waste_water_treated_volume falsy) the
#     readings are reported as 0 rather than the raw stored value, matching
#     the previous sample-based behavior.
#
...
def get_influent_parameter_map(dor_names):
    ...
    now live on the DOR itself rather than on a separate Water Sample doc).

    Batched: one query pulls every Influent Parameters Table row across
    ALL dor_names, rather than querying per-DOR."""
    if not dor_names:
        return {}

    rows = frappe.get_all(
        "Influent Parameters Table",
        filters={"parent": ["in", dor_names], "parenttype": "Daily Operation Report"},
        fields=["parent", "parameter", "actual_value"],
    )

    result = {}
    for r in rows:
        result.setdefault(r.parent, {})[r.parameter] = r.actual_value
    return result


def get_effluent_parameter_map(dor_names):
    """dor.name -> {parameter: actual_value}, built from Effluent
    Parameters Table rows attached directly to that DOR. Same batching
    pattern as get_influent_parameter_map()."""
    if not dor_names:
        return {}
...
def get_columns(filters):
    ...
    if not filters.get("hide_energy_consumption"):
    ...
    # Inlet parameter columns (filtered by "parameter"). Values are pulled
    # from Influent Parameters Table rows on the DOR itself - see
    # get_influent_parameter_map.
    ...
    # Outlet parameter columns - always ALL_PARAMETERS. Values are pulled
    # from Effluent Parameters Table rows on the DOR itself - see
    # get_effluent_parameter_map.
...
def get_data(filters, ctx):
    ...
    settings_cache = {}
    tier_cache = {}
    dor_names = [d.name for d in dor_list]

    # Batched once for every DOR in range - see get_fresh_water_consumption_map,
    # get_influent_parameter_map, get_effluent_parameter_map.
    fresh_water_map = get_fresh_water_consumption_map(dor_names)
    influent_map = get_influent_parameter_map(dor_names)
    effluent_map = get_effluent_parameter_map(dor_names)
    ...
    for dor in dor_list:
        ...
        # Inlet values pulled from Influent Parameters Table whenever
        # displayed OR whenever the off-spec calc needs COD (calc always
        # keys off Inlet, even if display is Outlet-only). Outlet values
        # pulled from Effluent Parameters Table.
        inlet_values = influent_map.get(dor.name, {}) if (ctx.show_inlet or not ctx.hide_calc) else {}
        outlet_values = effluent_map.get(dor.name, {}) if ctx.show_outlet else {}

        # Preserve previous sample-based behavior: on a zero-flow day,
        # readings are reported as 0 rather than the raw stored value.
        if not dor.waste_water_treated_volume:
            inlet_values = {k: 0 for k in inlet_values}
            outlet_values = {k: 0 for k in outlet_values}
        ...
        # Off-spec calculation - always driven by the Inlet (Influent) COD value.
